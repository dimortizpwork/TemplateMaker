...
using InviteToKill.Oracle.Adapter.Dto;
using InviteToKill.Oracle.Adapter.Helpers;
using InviteToKill.Repositories;

namespace InviteToKill.Oracle.Adapter.Repositories
{
    public class InviteToKillRepository : AbstractOracleRepository, IInviteToPayRepository
    {
        ...
        public InviteToKillRepository(ITimingDbConnectionFactory connectionFactory, OracleResiliencePolicy resiliencePolicy) : base(
        ...
        public InviteToKillModel Get(long InviteToPayId)
        {
            var parameters = new DynamicParameters();
            parameters.Add("P_ID", InviteToPayId);
            ...
            return ExecuteWithPolicy(() => {
                Connection.Open();
                var dto = Connection.QueryFirst<InviteToKillDto>("GetQuery", GetQuery, parameters);
                return dto.ToModel();
            }, MonitoringGetInviteToPay);
        }

        public int Post(InviteToKillModel model)
        {
            var parameters = new DynamicParameters();
        ...
        public void Put(long inviteToPayId, InviteToKillModel model)
        {
            var parameters = new DynamicParameters();
            parameters.Add("P_ID", inviteToPayId);
            parameters.Add("P_INVITETOPAYSTATEID", model.InviteToPayStateId);
